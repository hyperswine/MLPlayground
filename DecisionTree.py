...
param_grid = {'min_samples_leaf': np.arange(
    2, 30, 2), 'criterion': ['gini', 'entropy']}
grid_tree = GridSearchCV(tree.DecisionTreeClassifier(
...
print(r"New decision tree score for is {} (Grid search should be the same as before)".format(
    tree_performance))

# Maybe Grid Search isnt the best, try randomized search next
grid_tree = RandomizedSearchCV(
    tree.DecisionTreeClassifier(random_state=0), param_grid)
grid_tree.fit(X_train, y_train)

estimator = grid_tree.best_estimator_
y_pred = grid_tree.predict(X_test)
tree_performance = accuracy_score(y_test, y_pred)
print(r"New decision tree score for is {} (Randomized Search should be 78%)".format(
    tree_performance))
